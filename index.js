...
const bday = new Date('Feb 23, 2000')
...
console.log(age/1000, 'seconds\n', age/1000/60, 'minutes\n', age/1000/60/60, 'hours\n', age/1000/60/60/24, 'days\n', age/1000/60/60/24/365.25, 'years')
console.log('-------- BDay --------')
...
function consecutiveDates(date, repeat, offset) {
  ...
  let cycles = 0
  while(cycles < repeat){
    const newDate = new Date(date.getDate() + cycles*offset)
    dates.push(newDate)
    cycles += 1
  }
  return dates
...
function nextDate(dates) {
  ...
  let today = new Date()
  dates.push(today)
  let sortedDates = orderDates(dates)
  return sortedDates[sortedDates.indexOf(today) + 1]
}
...
function whensYourParty(date, year) {
  ...
  let thisBday = new Date(`Feb 23, ${today.getFullYear()}`)
  console.log(days[thisBday.getDay()])
}
